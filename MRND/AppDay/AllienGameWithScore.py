...
class Shot(Actor):
    ...
    def __init__(self, player,angle):
        ...
        self.image=pygame.transform.rotate(Img.shot,angle-90)
        self.rect=self.image.get_rect()
        self.vx=math.cos(math.radians(-angle))*SHOT_SPEED
        self.vy = math.sin(math.radians(-angle)) * SHOT_SPEED
...
class Score(Actor):
    def __init__(self):
        global SCORE
        Actor.__init__(self, Img.score)
        self.score=SCORE
        self.font=pygame.font.Font("freesansbold.ttf", 32)
    def update(self):
        global SCORE
        if(self.score!=SCORE):
            self.image=self.font.render(str(SCORE )+"     ",True,Color('White'))
            self.score = SCORE
            if(SCORE%4==0):
            ...
            if (SCORE % 4 == 2):
               self.rect.center = (600,40)
            if (SCORE % 4 == 3):
               self.rect.center = (400,40)
...
def main():
    ...
    Img.score=f.render(str(0)+"     ",True,Color('White'))
    angle=90
    ...
    while player.alive or explosions:
        ...
        #score.font.render(str(SCORE),0,(0,0,0))
        for actor in [player] + aliens + shots + explosions+[score]:
